...
app = Flask(__name__)
# Gerando uma chave secreta segura para sessões. Em produção, use uma variável de ambiente.
...
mongo_uri = "mongodb+srv://juliocardoso:z5fTsL8EAgD8SORa@dbbhub.nxcw2n9.mongodb.net/?retryWrites=true&w=majority"
...
def update_profile():
    ...
    if processed_new_cnpj != current_cnpj:
        if processed_new_cnpj: # Se um novo CNPJ foi fornecido e não está vazio
            ...
            if cnpjs_collection.find_one({'cnpj': processed_new_cnpj, 'user_id': {'$ne': ObjectId(user_id)} if user_id else {'$exists': False}}): # Added conditional for user_id
